...
class EditNoteCubit extends Cubit<EditNoteState>{
  ...
  Future<void> updateData({required String id,required String imageName})async{
    emit(EditDataLoadingState());
    try{
      if(file!=null){
        await FirebaseStorage.instance.ref('images/$imageName').putFile(file!);
        String editedUrl=await FirebaseStorage.instance.ref('images/$imageName').getDownloadURL();
        await notesCollection.doc(id).set({
          'title': title,
          'note': note,
          'imageURL': editedUrl,//must written to make project sense by edited image because path not change
          'userUid':FirebaseAuth.instance.currentUser!.uid,
